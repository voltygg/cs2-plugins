...
- For `cs2-plugins`, validate root and `voltmod` independently. Relevant existing checks include `uv run poe lint`, `uv run poe test`, and VoltMod's `uv run poe lint`.
