...
Three cards top-left and a toast at the bottom. `Contracts::ServerCard` is the plugin's own,
the configured name over a live player count; the other two are for other plugins. admin-system draws its
...
`ui_hud_demo <slot>` (server console, -1 for everyone) fills the other two cards with
