...
{
public:
    /** Insert or refresh this server's registry row (called once at boot). */
    bool Upsert(const std::string& tag, const std::string& name);

    /** Advance last_seen so operators can tell which registered servers are alive. */
    bool Heartbeat(const std::string& tag);
};
...
{
public:
    /** steamId -> group names granted on @p serverTag. */
