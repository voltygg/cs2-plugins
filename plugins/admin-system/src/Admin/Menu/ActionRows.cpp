...
using VoltMod::EnabledCondition;
...
MenuItem ActionRows::Action(std::string_view labelKey, const Actions::Action& action)
{
...
std::shared_ptr<VoltMod::Menu> ActionRows::BuildPicker(const Effects::EffectDescriptor& effect,
                                                       EnabledCondition allowed) const
{
    ...
    for (const auto& choice : effect.Choices ? effect.Choices() : std::vector<Effects::EffectChoice>{})
    {
