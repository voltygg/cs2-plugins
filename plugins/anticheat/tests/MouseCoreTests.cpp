...
struct MouseHarness
{
    ...
    void Command(int dx, float yaw, uint64_t buttons = 0, bool scoped = false)
    {
        ...
        cmd.Scoped = scoped;
        cmd.EyePos = Eye;
        if (Core.OnSimulated(Observer, cmd, Tick, false, Now))
            ++Findings;
    }
...
TEST_CASE("Turns the mouse counts cannot explain that land on an enemy add up to a finding")
{
    MouseHarness h;
    h.Calibrate();
    CHECK(h.Core.Calibrated(Observer));
    ...
    CHECK(h.Findings == 0);
    CHECK(h.Core.Score(Observer, Now) == 5);
    h.Command(0, 5.0f);
    CHECK(h.Findings == 1);
}
...
TEST_CASE("A client whose counts never agree with its turns is never judged")
{
    ...
    CHECK_FALSE(h.Core.Calibrated(Observer));
    for (int i = 0; i < 6; ++i)
    {
        h.Command(0, 5.0f);
        h.Command(0, 0.0f);
