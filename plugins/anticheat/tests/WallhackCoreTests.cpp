...
using Anticheat::WallhackCore;
using Anticheat::WallhackShotContext;
...
struct WallhackHarness
{
    ...
    WallhackCore Core{Correlator};
    LagEstimate Lag = EstimateVisualLag(0.0f, 0.0f);  // one tick behind
    int32_t Tick = 0;
    ...
    void Step(const AimAngles& aim)
    {
        ...
        if (Core.OnFrame(Observer, Tick, true, Lag, Now))
            ++Findings;
        ++Tick;
    }
    ...
    std::optional<Anticheat::Finding> Hit(int32_t fireTick, bool headshot = false, bool teamSaw = false)
    {
        ...
        return Core.OnShot(Observer, shot, WallhackShotContext{.TeamSawVictim = teamSaw}, Now);
    }
};
...
TEST_CASE("Following a hidden enemy through a wall for a second is an episode, three are a finding")
{
    ...
    CHECK(h.Core.IsTracking(Observer));
    h.Break();
    h.Follow(70);
    CHECK(h.Core.Score(Observer, Now) == 4);
    h.Break();
    h.Follow(70);
    CHECK(h.Findings == 1);
...
TEST_CASE("Following an enemy in plain view is aim, not a wallhack")
{
    ...
    h.Follow(70);
    CHECK(h.Core.Score(Observer, Now) == 0);
    CHECK_FALSE(h.Core.IsTracking(Observer));
}
...
TEST_CASE("A shot through cover at a silent, unseen, unmoving enemy is evidence")
{
    ...
    {
        h.Hit(fireTick, true);
        CHECK(h.Core.Score(Observer, Now) == 3);
    }
    ...
    {
        h.Core.OnWeaponFire(Target, fireTick - 10);
        h.Hit(fireTick);
        CHECK(h.Core.Score(Observer, Now) == 0);
    }
}
