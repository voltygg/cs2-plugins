...
void ShotHistory::OnCommand(int slot, const CmdSample& cmd)
{
...
void ShotHistory::OnSimulated(int slot, int32_t cmdNum, int32_t serverTick, const Vec3& eyePos, bool airborne)
{
...
void ShotHistory::CaptureFrame(int32_t serverTick, const std::array<PositionSample, MaxSlots>& players)
{
    PositionFrame frame;
...
void ShotHistory::Prune(int32_t serverTick)
{
    for (auto& data : _slots)
    {
...
ShotView* ShotHistory::MatchEvent(int slot, std::string_view weapon, int32_t serverTick)
{
    if (!InSlotRange(slot))
        return nullptr;
...
ShotView* ShotHistory::OnBulletImpact(int slot, const Vec3& impact, int32_t serverTick)
{
...
ShotView* ShotHistory::OnPlayerHurt(int attackerSlot, int victimSlot, bool headshot, int32_t serverTick)
{
...
const PositionFrame* ShotHistory::FindFrame(int32_t serverTick) const
{
...
std::optional<PositionSample> ShotHistory::FindPosition(int32_t serverTick, int slot) const
{
...
bool ShotHistory::AreOpponents(int teamA, int teamB, bool teammatesAreEnemies)
{
...
std::deque<ShotView>& ShotHistory::Shots(int slot)
{
