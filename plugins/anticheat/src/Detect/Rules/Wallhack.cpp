...
void Wallhack::OnSimulated(int slot, int32_t serverTick, const AimAngles& angles, const Vec3& eyePos)
{
    if (!InSlotRange(slot))
        return;
...
void Wallhack::OnWeaponFire(int slot, int32_t fireTick)
{
    if (InSlotRange(slot))
        _slots[slot].LastFireTick = fireTick;
}

void Wallhack::CloseTrack(SlotData& data, int32_t serverTick, bool becameVisible)
{
    ...
    if (track.Qualified)
        data.CooldownUntilTick = serverTick + QuietTicks;
    track = {};
}
