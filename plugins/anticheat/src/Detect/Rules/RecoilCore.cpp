...
void RecoilCore::Reset()
{
    _slots = {};
    _incidents = {};
}

void RecoilCore::OnSlotChanged(int slot)
{
    if (!InSlotRange(slot))
        return;
    _slots[slot] = {};
...
void RecoilCore::OnCommand(int slot, const CmdSample& cmd)
{
...
std::optional<Finding> RecoilCore::OnFrame(int slot, int32_t serverTick, double nowSec)
{
    if (!InSlotRange(slot))
        return std::nullopt;
    auto& data = _slots[slot];
...
SprayFit RecoilCore::Fit(const SlotData& data, int viewLag) const
{
    struct Pair
    {
...
std::optional<Finding> RecoilCore::Finalize(int slot, SlotData& data, double nowSec)
{
    std::optional<Finding> out;
...
int RecoilCore::Score(int slot, double nowSec) const
{
    return InSlotRange(slot) ? _incidents[slot].Value(nowSec) : 0;
}

bool RecoilCore::InSpray(int slot) const
{
