...
/** A shot older than this has received every event it can, so the rules may judge it. */
...
void DetectionFeed::OnCommand(int slot, const VoltMod::PlayerInput& cmd)
{
    ...
    d.History.OnCommand(slot, sample);
...
void DetectionFeed::OnFrame()
{
    ...
    d.History.Prune(serverTick);
}

bool DetectionFeed::TeamSawVictim(int shooter, int victim, int32_t fireTick) const
{
    if (!IsValidSlot(shooter) || !IsValidSlot(victim))
        return false;

    const ShotHistory& frames = _detectors.History;
...
void DetectionFeed::FinalizeShots(int slot, int32_t serverTick, double nowSec)
{
    Detectors& d = _detectors;
...
void DetectionFeed::OnWeaponFire(const VoltMod::WeaponFire& fire)
{
    ...
    ShotView* shot =
        d.History.OnWeaponFire(fire.Slot, fire.Weapon, serverTick, visible, hasVisible, fireCmd, shotsFired);
    if (!shot)
        return;
    if (d.RuleEnabled(d.AntiAim))
...
void DetectionFeed::OnBulletImpact(const VoltMod::BulletImpact& impact)
{
    ...
    int slot = d.History.ResolveImpactShooter(impact.TruncatedUserId, serverTick, _userIds);
    if (slot < 0 && !_userIdsResolved)
        slot = impact.Slot;
    if (!d.IsEligible(slot))
        return;

    ShotView* shot = d.History.OnBulletImpact(slot, {impact.X, impact.Y, impact.Z}, serverTick);
    if (shot && d.RuleEnabled(d.SilentAim))
        d.SilentAim.OnShotUpdated(slot, *shot);
}

void DetectionFeed::OnPlayerHurt(const VoltMod::PlayerHurt& hurt)
{
    Detectors& d = _detectors;
    if (!d.Enabled())
        return;
    ...
    ShotView* shot = d.History.OnPlayerHurt(attacker, victim, headshot, static_cast<int32_t>(_rt.Clock.Tick()));
    if (!shot)
        return;

    const double now = Time::MonotonicSeconds();
    if (d.RuleEnabled(d.SilentAim))
