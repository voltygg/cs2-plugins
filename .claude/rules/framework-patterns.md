...
- For an engine function the framework does not cover: `HookInterface`, `HookClassSlot` or `HookFunction` from `<VoltMod/Unsafe/Hook.hpp>`, keeping the `Subscription` it returns. A handler is a lambda taking the hooked object first; a before-handler returns `HookResult` or nothing, an after-handler observes.
...
- `runtime.Screens.Shared(layout)` and `runtime.Screens.ForPlayer(layout, slot)` return a move-only `Screen` whose destructor removes the entity; hold it as a member. `runtime.Addons.Require(id)` returns a `Subscription`; the requirement lasts as long as you hold it.
...
- Open and close menus through `app.Menus` (`Menus::MenuRouter`): a session starts on the Panorama screen when the player can see it and on `runtime.Menus` (center text) otherwise, and every later call follows that session.
