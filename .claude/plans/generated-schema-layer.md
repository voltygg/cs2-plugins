...
- CLI: `vendor/voltmod/cli/voltmod/cli.py` (Typer; sub-Typer precedent `builder/package.py`, `app.add_typer(...)` at :22); artifact producers live in `builder/`; pytest in `cli/tests/` (`pythonpath=["cli"]`, run by `poe test-tools`, part of `poe lint`); codegen precedent `scaffold/new_plugin.py` (`write_text(..., newline="\n")`). modgraph allowlist `cli/voltmod/checks/modgraph.py`; `cli/tests/test_modgraph.py:124-128` asserts the fenced layering blocks in voltmod `CLAUDE.md` and `docs/architecture.md` match.
...
- New consumer-repo plugin `plugins/schema-dump/` (`uv run poe new-plugin schema-dump`; dev-only, never deployed). Registers `schema_dump [path]` through the normal command builder; reaches the schema system through `runtime.Unsafe.Interfaces.SchemaSystem` plus `<schemasystem/schemasystem.h>` / `<schemasystem/schematypes.h>` directly. Nothing enters `voltmod-runtime`; command ownership is unambiguous.
...
### CLI — `cli/voltmod/builder/schemagen.py`, sub-Typer wired in `cli.py`
...
### Tests — `cli/tests/test_schemagen.py` (pytest; under `poe lint`)
...
- Add a `"schema"` status section (verified class/field counts) beside "load"/"gamedata"/"capabilities"; remove `Capability::Schema` (no plugin queries it any more - `ui-lab` was removed and `plugins/anticheat` now checks only `ClientCvars`/`Teleport` - so this is a pure deletion).
