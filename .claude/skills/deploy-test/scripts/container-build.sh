...
# The checkout's own CLI: the pinned one may link an editable framework without rebuilding it.
uv venv --quiet --allow-existing --python 3.14 /cache/venv
uv pip install --quiet --python /cache/venv/bin/python --reinstall-package voltmod /work/vendor/voltmod
export PATH="/cache/venv/bin:$PATH"

conan profile detect --exist-ok >/dev/null 2>&1 || true
conan config install /work/vendor/voltmod -sf conan >/dev/null
...
if [ "$FRAMEWORK" = checkout ]; then
    conan editable add /work/vendor/voltmod
fi

voltmod build "$preset"
if [ "$RUN_TESTS" = 1 ]; then
    voltmod test "$preset"
fi
...
if [ "$FRAMEWORK" = checkout ]; then
    cmake --install "/work/vendor/voltmod/build/$preset" --component host --prefix /work/package/host
fi
