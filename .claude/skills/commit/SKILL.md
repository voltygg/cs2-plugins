...
revision (`conan.lock`). The kit checkout is wherever `conan editable list`
points, usually `vendor/voltmod`.

## Steps

1. **Inspect both repos** in parallel: `git status`, `git diff HEAD --stat`,
   `git log --oneline -5`, and the same with `git -C <kit>`. Both clean: say so
   and stop. Skip any repo with no changes.
2. **voltmod first.** Stage by file name, commit, `git -C <kit> push origin main`.
...
3. **cs2-plugins.** Stage by file name, commit, `git push origin main`.
   ...
   After a kit push, refresh with `uv lock --upgrade-package voltmod`.
